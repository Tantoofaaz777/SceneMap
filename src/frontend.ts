...
function openRegenerationModal() {
  ...
  const syncControls = () => {
    ...
    const feedbackField = root.querySelector<HTMLElement>(".scenemap-regeneration-feedback");
    const feedbackInput = root.querySelector<HTMLTextAreaElement>("[data-regeneration-feedback]");
    if (feedbackField) feedbackField.hidden = mode !== "fields";
    if (feedbackInput) feedbackInput.disabled = mode !== "fields";
  ...
  const handleModalClick = (event: Event) => {
    ...
    const feedback = root.querySelector<HTMLTextAreaElement>("[data-regeneration-feedback]")?.value ?? "";
    if (paths.length === 0) {
      syncControls();
      return;
    }
